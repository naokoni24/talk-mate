...
- UIを相談サービスらしい淡い紫基調に刷新。ペルソナカードの視認性、通話中の状態・字幕、結果、履歴の情報階層を改善し、390px幅でも横スクロールなしで表示
- 金額は米ドルに加えて日本円の概算を併記。本日の利用額、通話終了結果、履歴が対象
- 円換算は `USD_JPY_RATE`（既定160.0円）を使用し、結果画面には換算レートも表示
...
画面の日本円は請求額ではなく概算。`USD_JPY_RATE` の設定値でドル額を換算し、1円単位に四捨五入する。
